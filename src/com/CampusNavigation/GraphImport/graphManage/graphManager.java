...
public abstract class graphManager {
  ...
  public static Graph manage(AssetManager assetManager, String path) throws IOException {
    ...
    while (scanner.hasNextLine() && !now.contains("/* draw figures */")) {
      now = scanner.nextLine();
    }
    ;

    for (; scanner.hasNextLine() && !now.contains("/* dots and labels */");
        now = scanner.nextLine()) {
    ...
    for (; scanner.hasNextLine() && !now.contains("/* end of picture */");
      ...
      if (justGetOne && now.contains("label")) {
        ...
        if (dotPosition == null || dotX == null || dotY == null) {
          continue;
        }
        if (GuiPoints.containsKey(dotPosition)) {
          continue;
        }
    ...
    for (; scanner.hasNextLine() && !now.contains("/* dots and labels */");
      ...
      if (now.contains("draw") && !now.contains("circle") && !now.contains("EndArrow")) {//道路
        ...
        if (dotPosition2 == null || dotPosition1 == null || widthStr == null) {
          continue;
        }
        Double width = Double.parseDouble(widthStr);

        if (!allDots.containsKey(dotPosition1) || !allDots.containsKey(dotPosition2)) {
        ...
        if (now.contains("qqccqq")) {
          edges[index1][index2].setBikeOk(true);
          edges[index2][index1].setBikeOk(true);
        } else if (now.contains("qqzzff")) {
          edges[index1][index2].setBikeOk(false);
          edges[index2][index1].setBikeOk(false);
        }
        ...
        allDots.get(dot1).xg = Double
            .parseDouble(matcher.theFirst("(?<=\\()[0-9.-]+(?=,[0-9.-]+\\))"));
        allDots.get(dot1).yg = Double
            .parseDouble(matcher.theFirst("(?<=\\([0-9.-]{1,50},)[0-9.-]+(?=\\))"));
        allDots.get(dot1).setRg();
      } else if (now.contains("draw") && (now.contains("linetype") || now.contains("linewidth"))
          && now.contains("circle")) {//路口
        MyMatcher matcher = new MyMatcher(now);
        Dot theDot;
        String type = "";
  ...
  private static void setDotType1(Dot theDot, String type) {
    ...
    switch (type) {
      case "ffccww":
        theDot.setType(BuildingType.buildingCrossing);
        break;
      case "aqaqaq":
        theDot.setType(BuildingType.car);
        break;
      case "ffwwqq":
        theDot.setType(BuildingType.bus);
        break;
      case "ffwwzz":
        theDot.setType(BuildingType.runway);
        break;
      case "zzwwff":
        theDot.setType(BuildingType.soccer);
        break;
      /////
      case "ttffcc":
        theDot.setType(BuildingType.crossing);
        break;
      /////
      case "wwffqq":
        theDot.setType(BuildingType.exit);
        break;
      /////
      default:
        theDot.setType(BuildingType.buildingCrossing);
  ...
  private static void setDotType2(Dot theDot, String type) {
    ...
    switch (type) {
      case "blue":
        theDot.setType(BuildingType.lake);
        break;
      case "cczzff":
        theDot.setType(BuildingType.studentCenter);
        break;
      case "ffwwzz":
        theDot.setType(BuildingType.canteen);
        break;
      case "ffwwqq":
        theDot.setType(BuildingType.tree);
        break;
      case "ttttff":
        theDot.setType(BuildingType.swim);
        break;
      case "ttffcc":
        theDot.setType(BuildingType.service);
        break;
      case "wwffqq":
        theDot.setType(BuildingType.hospital);
        break;
      case "ffcctt":
        theDot.setType(BuildingType.office);
        break;
      case "red":
        theDot.setType(BuildingType.teach);
        break;
      case "qqzzff":
        theDot.setType(BuildingType.dorm);
        break;
      case "cqcqcq":
        theDot.setType(BuildingType.lab);
        break;
      case "ffefdv":
        theDot.setType(BuildingType.coffee);
        break;
      case "ffcqcb":
        theDot.setType(BuildingType.librariy);
        break;
      default:
        theDot.setType(BuildingType.Default);
        break;
    }
  }
